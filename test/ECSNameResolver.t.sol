...
contract ECSNameResolverTest is Test {
    /* --- Constants --- */
    ...
    // Removed _createNameDNSName and helper functions - using NameCoder.encode directly instead
    ...
    function test1100____________________ECS_NAME_RESOLVER_TESTS______________________________() public pure {}
    ...
    function test_006____setCredentialResolver_______SetsResolverSuccessfully() public {
        ...
        emit ECSNameResolver.CredentialResolverRegistered(namespace1Hash, "label1.ecs.eth", address(mockResolver1));
    ...
    function test_008____setCredentialResolver_______RevertsForUnauthorized() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ECSNameResolver.UnauthorizedNamespaceAccess.selector,
                user2,
                namespace1Hash
            )
        );
    ...
    function test_009____setCredentialResolver_______AllowsZeroAddressToRemove() public {
        ...
        emit ECSNameResolver.CredentialResolverRemoved(namespace1Hash, "label1.ecs.eth");
    ...
    function test_010____setCredentialResolver_______RevertsForExpiredNamespace() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ECSNameResolver.NamespaceExpired.selector,
                shortNamespace
            )
        );
    ...
    function test_011____setCredentialResolver_______UpdatesResolverSuccessfully() public {
        ...
        vm.startPrank(user1);
        vm.expectEmit(true, true, false, true);
        emit ECSNameResolver.CredentialResolverRegistered(namespace1Hash, "label1.ecs.eth", address(mockResolver2));
    ...
    function test_015____resolve_____________________ResolvesWithSingleNamespace() public {
        ...
        // Test resolution with domain name
        bytes memory name = NameCoder.encode("example.com.name.ecs.eth");
    ...
    function test_016____resolve_____________________FindsLongestMatchingNamespace() public {
        ...
        string memory decodedResult = abi.decode(result, (string));
        assertEq(decodedResult, "mock-response-1");
        // We should resolve the credential resolver for the longest matching namespace
    }

    function test_017____resolve_____________________FindsLongestMatchingNamespaceECSNode() public {
        // Test longest matching namespace with nested credential namespaces
        // First register the credential subname under label1.ecs.eth
        vm.startPrank(user1);
        registry.setSubnameOwner("credential", "label1.ecs.eth", user1, block.timestamp + REGISTRATION_DURATION, false);
        vm.stopPrank();
    ...
    function test_018____resolve_____________________FindsLongestMatchingNamespaceETHNode() public {
        // Test longest matching namespace with different credential key
        // First register the credential subname under label1.ecs.eth
        vm.startPrank(user1);
        registry.setSubnameOwner("credential", "label1.ecs.eth", user1, block.timestamp + REGISTRATION_DURATION, false);
        vm.stopPrank();
    ...
    function test_019____resolve_____________________ReturnsEmptyForNoMatch() public {
        ...
        // Test resolution with domain name but no resolver set
        bytes memory name = NameCoder.encode("example.com.name.ecs.eth");
        bytes memory data = abi.encodeWithSelector(
            bytes4(0x59d1d43c), // text(bytes32,string) selector
            bytes32(0), // dummy node hash (ignored)
    ...
    function test_022____resolve_____________________HandlesOffchainLookup() public {
        // First register the credential subname under label1.ecs.eth
        vm.startPrank(user1);
        registry.setSubnameOwner("credential", "label1.ecs.eth", user1, block.timestamp + REGISTRATION_DURATION, false);
        vm.stopPrank();
    ...
    function test_023____onCredentialSuccess_________ReturnsDecodedResult() public view {
    ...
    function test_024____onCredentialFailure_________RethrowsOriginalError() public {
    ...
    function test_025____supportsInterface___________ReturnsCorrectInterfaceSupport() public view {
    ...
    function test_026____approvalWorkflow____________ManagesApprovalAndOperatorActions() public {
        // First register the credential subname under label1.ecs.eth
        vm.startPrank(user1);
        registry.setSubnameOwner("credential", "label1.ecs.eth", user1, block.timestamp + REGISTRATION_DURATION, false);
        vm.stopPrank();
        
        // User1 approves operator1 for credential namespace via registry
        vm.startPrank(user1);
    ...
    function test_027____namespaceExpiration_________PreventsExpiredNamespaceOperations() public {
        // Register namespace with short duration
        uint256 fee = controller.calculateFee(SHORT_DURATION);
        vm.startPrank(user1);
        bytes32 shortNamespace = controller.registerNamespace{value: fee}("shortlived", SHORT_DURATION);
        ...
        // Cannot register new resolver - should check credential namespace hash
        bytes32 credentialNamespaceHash = NameCoder.namehash(NameCoder.encode("credential.shortlived.ecs.eth"), 0);
        vm.expectRevert(
            abi.encodeWithSelector(
                ECSNameResolver.NamespaceExpired.selector,
                credentialNamespaceHash
        ...
        resolver.setCredentialResolver("credential.shortlived.ecs.eth", address(mockResolver2));
        
        // Cannot remove existing resolver - should check credential namespace hash
        vm.expectRevert(
            abi.encodeWithSelector(
                ECSNameResolver.NamespaceExpired.selector,
                credentialNamespaceHash
        ...
        resolver.setCredentialResolver("credential.shortlived.ecs.eth", address(0));
    ...
    function test_028____errorHandling_______________HandlesCredentialResolverErrors() public {
        // First register the credential subname under label1.ecs.eth
        vm.startPrank(user1);
        registry.setSubnameOwner("credential", "label1.ecs.eth", user1, block.timestamp + REGISTRATION_DURATION, false);
        vm.stopPrank();
    ...
    function test_029____domainNameFormats___________SupportsDifferentDomainFormats() public {
        // First register the credential subname under label1.ecs.eth
        vm.startPrank(user1);
        registry.setSubnameOwner("credential", "label1.ecs.eth", user1, block.timestamp + REGISTRATION_DURATION, false);
        vm.stopPrank();
        ...
        // Set up resolver for credential namespace
        vm.startPrank(user1);
        resolver.setCredentialResolver("credential.label1.ecs.eth", address(mockResolver1));
        vm.stopPrank();
        
        // Test different domain name formats
        string[] memory testDomains = new string[](4);
        testDomains[0] = "example.com.name.ecs.eth";
        testDomains[1] = "subdomain.example.com.name.ecs.eth";
        testDomains[2] = "test.org.name.ecs.eth";
        testDomains[3] = "deep.nested.domain.co.uk.name.ecs.eth";
        ...
        for (uint256 i = 0; i < testDomains.length; i++) {
            bytes memory name = NameCoder.encode(testDomains[i]);
            bytes memory result = resolver.resolve(name, data);
            string memory decodedResult = abi.decode(result, (string));
            assertEq(decodedResult, "mock-response-1");
        }
